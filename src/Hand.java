...
public class Hand {
    ...
    static void display(User player, User... bank){
        if(bank.length > 0){
            ...
            System.out.println(bank[0].getUserDeck().tostring());
        }
        System.out.println(player.getUsername() + " deck :");
        System.out.println(player.getUserDeck().tostring());
    ...
    static void bankTurn(User bank, DeckOfCards deck){
        bank.getUserDeck().ChangeVisible(Visible.Unfolded);
        display(bank);
        while (bank.getHandValue() < 17){
            bank.newCard(deck.pickRandom());
        }
        display(bank);
    }
